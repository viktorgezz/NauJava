package ru.viktorgezz.testing_system.domain.answer_option;
...
import ru.viktorgezz.testing_system.domain.answer_option.repo.AnswerOptionRepo;
import ru.viktorgezz.testing_system.domain.question.Question;
import ru.viktorgezz.testing_system.domain.question.repo.QuestionRepo;
import ru.viktorgezz.testing_system.domain.test.Status;
import ru.viktorgezz.testing_system.domain.test.TestModel;
import ru.viktorgezz.testing_system.domain.test.repo.TestRepo;
...
import static ru.viktorgezz.testing_system.util.CreationModel.*;

@DisplayName("AnswerOptionRepo Integration Tests")
class AnswerOptionRepoTest extends AbstractIntegrationPostgresTest {
    ...
    void findAllByIdsQuestionWithQuestion_ShouldReturnAnswerOptions_WhenQuestionsExist() {
        List<Long> idsQuestion = List.of(questionFirst.getId(), questionSecond.getId());

        List<AnswerOption> answerOptionsFound = answerOptionRepo.findAllByIdsQuestionWithQuestion(idsQuestion);
    ...
    void findAllByIdsQuestionWithQuestion_ShouldReturnEmptyList_WhenQuestionsDoNotExist() {
        List<Long> idsQuestionNonExistent = List.of(9999L, 10000L);

        List<AnswerOption> answerOptionsFound = answerOptionRepo.findAllByIdsQuestionWithQuestion(idsQuestionNonExistent);
    ...
    void findAllByIdsQuestionWithQuestion_ShouldReturnOnlySpecifiedQuestionsOptions_WhenMultipleQuestionsExist() {
        List<Long> idsQuestionSpecified = List.of(questionFirst.getId());

        List<AnswerOption> answerOptionsFound = answerOptionRepo.findAllByIdsQuestionWithQuestion(idsQuestionSpecified);
