<!--
  Модальное окно для создания нового теста.
  Реализует валидацию полей согласно требованиям бэкенда (TestMetadataDto)
  и интеграцию с API для создания теста.
-->
<template>
  <div v-if="isOpen" class="modal-overlay" @click.self="handleClose">
    <div class="modal-container">
      <div class="modal-header">
        <h2 class="modal-title">Создать тест</h2>
      ...
      <form @submit.prevent="handleSubmit" class="modal-form">
        ...
        <div class="form-group">
          ...
          <div class="status-display">
            <span class="status-badge status-private">Приватный</span>
            <span class="status-hint">Статус нельзя изменить</span>
          </div>
        </div>
...
import { createTestMetadata } from '@/api/testService'
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const testId = await createTestMetadata({
    ...
    if (testId) {
      ...
      submitError.value = 'Ошибка при создании теста'
    }
  } catch (error) {
    submitError.value =
      error.response?.data?.message || error.message || 'Произошла ошибка при создании теста'
  } finally {
