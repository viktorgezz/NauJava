...
class TestRepoImplTest extends AbstractIntegrationPostgresTest {
    ...
    void setUp() {
        ...
        javaTest = testRepo.save(savingModel.createTest("Java Advanced", "Advanced Java topics", Status.PUBLIC, author1));
        springTest = testRepo.save(savingModel.createTest("Spring Test", "Spring topics", Status.PRIVATE, author1));
        sqlTest = testRepo.save(savingModel.createTest("SQL Basics", "Basic SQL queries", Status.PUBLIC, author1));
        combinedTest = testRepo.save(savingModel.createTest("Java & Spring", "Combined topics", Status.PUBLIC, author1));
    ...
    void findByTitle_Criteria_shouldReturnMatchingTests() {
        ...
        testRepo.save(savingModel.createTest(targetTitle, "Another one", Status.UNLISTED, author1)); // Добавим еще один с тем же названием
    ...
    void findByTitle_CriteriaAndDerivedShouldMatch() {
        // Arrange
        String targetTitle = "Java Advanced";
        testRepo.save(savingModel.createTest(targetTitle, "Another one", Status.UNLISTED, author1));
